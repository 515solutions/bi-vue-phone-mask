<template>
    <div :class="wrapperClass">
        <the-mask :mask="mask" :value="innerValue" v-on:input="onInput" :tokens="plusTokens" :placeholder="placeholder" :disabled="disabled" masked type="tel" :class="inputClass" ref="phoneMask"/>
...
import {getMaskToLibPhoneNumber,findFirstCountryByAlpha2,findFirstCountryByCode,isCanada} from "./country_telephone_data.js";
...
export default {
    ...
    computed: {
        currentNumber: function(){
            ...
            const filteredNumberArr = this.innerValue ? this.innerValue.match(/[\d+]/g) : null;
            const filteredNumber = filteredNumberArr ? filteredNumberArr.join("") : "";
            return this.mask === this.defaultMask ? `${plus}${filteredNumber}` : filteredNumber;
        }
    },
    methods: {
        ...
        updateMaskData: function(){
            let {currentNumber,visitorCountry,innerValue,countryCode,defaultMask,autoDetectCountry,defaultCountry} = this;
    ...
    beforeMount() {
        if(this.autoDetectCountry){
            ...
            this.visitorCountry = visitorContryInfo && visitorContryInfo.country ? visitorContryInfo.country.alpha2 : "";
        }
        this.updateMaskData();
    },
    mounted() {
...
<style scoped>
    .phone-mask-wrapper-lib{display:flex;flex-direction:row;align-items:center;height:40px;}
    .phone-input-lib{height:20px;}
    .country-flag-lib{margin:0;}
</style>